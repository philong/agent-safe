...
- Pi's IDE extension (`npm:pi-x-ide`) has the same problem without the escape hatch — it skips *and* deletes every lockfile in `~/.pi/pi-x-ide/lock` whose pid is not alive, and offers no port variable to bypass the test. Since `pid` is optional in its own lockfile schema and that liveness test is the only thing that reads it, `agent-safe` binds a sanitized snapshot over the lock directory: the lockfiles whose pid is alive on the host, copied with the pid dropped, so they are accepted on mtime freshness instead. The originals stay out of the sandbox's reach, stale lockfiles are left out rather than sanitized, and the snapshot lives in `$XDG_RUNTIME_DIR` for the lifetime of the session. It is a snapshot, so the same restart caveat applies.
